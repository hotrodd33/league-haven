  ...
  return (
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
        <h2 className="font-display text-2xl font-bold tracking-wide text-white">Teams</h2>
        <select
          value={filterOrg}
      ...
      {orgGroups.map(group => (
        <div key={group.org.name} className="mb-8">
          <div className="flex items-center gap-3 mb-3 border-b-2 border-action-700 pb-1.5">
            <TeamLogo src={group.org.logo} name={group.org.name} size="w-7 h-7" />
            <h3 className="font-display text-base font-bold text-action-300 tracking-wide">{group.org.name}</h3>
            <span className="text-xs text-gray-500 ml-auto">{group.teams.length} team{group.teams.length !== 1 ? 's' : ''}</span>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {group.teams.map(team => (
              ...
              >
                <TeamLogo
                  src={team.logo_url || group.org.logo}
                ...
                <div className="min-w-0">
                  ...
                  {team.divisions?.length > 0 && (
                    <div className="text-xs text-action-400 mt-0.5 truncate">
