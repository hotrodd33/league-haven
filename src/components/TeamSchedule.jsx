...
function GameCard({ game, teamId, weather, onSelect, onTrack, onSchedule, canScore }) {
  ...
  const missingScore = needsScoreEntry(game);
  ...
  const missingScoreTone = missingScore ? '!border-signal-500 ring-1 ring-signal-500/40' : '';

  return (
    <div onClick={onSelect}
      className={`${cardTone} ${missingScoreTone} border rounded-lg px-3 py-2 text-sm cursor-pointer hover:border-chrome-300 hover:shadow-sm transition-all`}>
